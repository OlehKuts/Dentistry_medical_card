...
export const Review = ({ current, params, onEditAdd }) => {
  ...
  const splittedExaminations = examinations
    ? examinations.split(",")
    : initialExaminations.split(",");
  ...
  const mandibulaStatusList = toothFormula
    ...
    .splice(tLength, tLength);
  const maxillaNames = toothFormula.map((item) => item.name).splice(0, tLength);
    ...
    <div id="review">
      ...
      <div className="flexi headers">
        <Block
          onEditAdd={onEditAdd}
          patientId={_id}
          header={`${stringCapitalizer(doctorType)}:`}
          content={doctor}
          editType="doctor"
        />
      </div>
      ...
      <Block
        onEditAdd={onEditAdd}
        patientId={_id}
        header="Анамнез життя:"
        content={anamnesisVitaeContent}
