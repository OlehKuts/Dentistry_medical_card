export const diseaseList = [
  { name: "Оберіть патологію", value: "" },
  { name: "періодонтит", value: "periodontit" },
  { name: "періостит", value: "periostit" },
  { name: "запальні процеси м'яких тканин", value: "abscess" },
  { name: "травма м'яких тканин", value: "skinWound" },
  { name: "вивих зуба", value: "dislocationTooth" },
  { name: "перелом нижньої щелепи", value: "fractureLowerJaw" },
  { name: "множинний карієс", value: "caries" },
  { name: "коротка вуздечка язика", value: "tongueBridle" },
  { name: "коротка вуздечка верхньої губи", value: "lipBridle" },
  { name: "новоутвори м'яких тканин", value: "neoplasm" },
  { name: "одонтогенні кісти щелеп", value: "cyst" },
  { name: "ретенція зубів", value: "retention" },
  { name: "надкомплектні зуби", value: "overComplete" },
];
export const diseaseCodesList = [
  { name: "Оберіть патологію", value: "" },
  { name: "множинний карієс", value: "множинний карієс" },
  { name: "коротка вуздечка язика", value: "коротка вуздечка язика" },
  { name: "коротка вуздечка верхньої губи", value: "коротка вуздечка губи" },
  { name: "новоутвори м'яких тканин", value: "новоутвір обличчя" },
  { name: "одонтогенні кісти щелеп", value: "кіста щелепи" },
  { name: "ретенція зубів", value: "ретенція зубів" },
  { name: "надкомплектні зуби", value: "надкомплектні зуби" },
  { name: "періодонтит", value: "періодонтит" },
  { name: "періостит", value: "періостит" },
  { name: "запальні процеси м'яких тканин", value: "абсцес обличчя" },
  { name: "травма м'яких тканин", value: "рана обличчя" },
  { name: "вивих зуба", value: "вивих зуба" },
  { name: "перелом нижньої щелепи", value: "перелом нижньої щелепи" },
];
export const doctors = [
  { name: "Особистий лікар", value: "" },
  { name: "ЛІКАР_1", value: "ЛІКАР_1" },
  { name: "ЛІКАР_2", value: "ЛІКАР_2" },
  { name: "ЛІКАР_3", value: "ЛІКАР_3" },

];
// export const otolaryngologists = doctors.slice(-4).map((item) => item.name);
export const start2List = ["день", "тиждень", "місяць", "рік"];
export const conditionList = [
  "задовільний",
  "близький до задовільного",
  "середнього ступеню важкості",
  "важкий",
];
export const crownDestructionList = [
  { name: "коронка зруйнована частково", value: "частково" },
  { name: "коронка зруйнована повністю", value: "повністю" },
];
export const analyses = [
  {
    name: "загальний аналіз крові + час згортання та тривалість кровотечі",
    feature: "Примітки",
    urgent: true,
  },
  { name: "загальний аналіз сечі", feature: "Примітки", urgent: true },
  {
    name: "аналіз крові на групу та резус-фактор ",
    feature: "Група крові",
    urgent: false,
  },
  { name: "глюкоза крові", feature: "Примітки", urgent: false },
];
export const drugName1List = [
  { name: "Антибіотик", value: "" },
  { name: "Амоксиклав", value: "Амоксиклав" },
  { name: "Амоксицилін", value: "Амоксицилін" },
  { name: "Лінкоміцин", value: "Лінкоміцин" },
  { name: "Сумамед", value: "Сумамед" },
  { name: "Цефодокс", value: "Цефодокс" },
  { name: "Лопракс", value: "Лопракс" },
  { name: "Цефинак", value: "Цефинак" },
  { name: "Цефтріаксон", value: "Цефтріаксон" },
  { name: "Цефазолін", value: "Цефазолін" },
  { name: "Діацеф", value: "Діацеф" },
  { name: "Ефмерин", value: "Ефмерин" },
  { name: "Цефотаксим", value: "Цефотаксим" },
  { name: "Аміцил", value: "Аміцил" },
  { name: "Цефікс", value: "Цефікс" },
  { name: "Оспамокс(250мг)", value: "Оспамокс" },
  { name: "Зитрокс (250мг)", value: "Зитрокс" },
];
export const drugName2List = [
  { name: "Протизапальний", value: "" },
  { name: "Нурофен", value: "Нурофен" },
  { name: "Парацетамол", value: "Парацетамол" },
];
export const drugName3List = [
  { name: "Пробіотик", value: "" },
  { name: "Неофлорум", value: "Неофлорум" },
  { name: "Пробіз", value: "Пробіз" },
  { name: "Ентерожерміна форте", value: "Ентерожерміна форте" },
  { name: "Євробіотик", value: "Євробіотик" },
];
export const drugName4List = [
  { name: "Антигістамінний", value: "" },
  { name: "Лоратадин", value: "Лоратадин" },
  { name: "Феністил", value: "Феністил" },
];
export const drugName5List = [
  { name: "Інші призначення", value: "" },
  { name: "Метрогіл", value: "Метрогіл" },
  { name: "Гепарин", value: "Гепарин" },
  { name: "L-лізину есцинат", value: "L-лізину есцинат" },
  { name: "Содовий розчин", value: "Содовий розчин" },
];
export const pliersList = [
  { name: "Оберіть щипці", value: "" },
  { name: "багнетоподібні", value: "багнетоподібних щипців" },
  { name: "дзьобоподібні", value: "дзьобоподібних щипців" },
  { name: "прямі", value: "прямих щипців" },
];
export const elevatorList = [
  { name: "Оберіть елеватор", value: "" },
  { name: "прямий", value: "прямого елеватора" },
  { name: "кутовий", value: "кутового елеватора" },
];
export const surgeons = [{ name: "Хірург", value: "" }].concat(
  doctors.slice(1)
);
export const assistants = [{ name: "Асистент", value: "" }].concat(
  doctors.slice(1)
);
export const anesthetistList = [
  { name: "Анестезіолог", value: "" },
  { name: "відсутній", value: " - " },
  { name: "АНЕСТЕЗІОЛОГ_1", value: "АНЕЗТЕЗІОЛОГ_1" },
  { name: "АНЕСТЕЗІОЛОГ_2", value: "АНЕЗТЕЗІОЛОГ_2" },
  { name: "АНЕСТЕЗІОЛОГ_3", value: "АНЕЗТЕЗІОЛОГ_3" },
];
export const areaList = [
  { value: "", name: "ділянка" },
  { value: "підщелепної ділянки", name: "підщелепна" },
  { value: "підборідної ділянки", name: "підборідна" },
  { value: "щічної ділянки", name: "щічна" },
  { value: "нижньої губи", name: "нижня губа" },
  { value: "верхньої губи", name: "верхня губа" },
  { value: "підочної ділянки", name: "підочна" },
  { value: "привушно-жувальної ділянки", name: "привушно-жувальна" },
  { value: "виличної ділянки", name: "вилична" },
  { value: "скроневої ділянки", name: "скронева" },
  { value: "лобної ділянки", name: "лобна" },
  { value: "брови", name: "брова" },
  { value: "шийної ділянки", name: "шийна" },
  { value: "позадувушної ділянки", name: "позадувушна" },
  { value: "твердого піднебіння", name: "тверде піднебіння" },
  { value: "м'якого піднебіння", name: "м'яке піднебіння" },
  { value: "під'язикової ділянки", name: "під'язикова" },
  { value: "язика", name: "язик" },
];
export const sideList = [
  { value: "", name: "сторона" },
  { value: " ліворуч", name: "ліва" },
  { value: " праворуч", name: "права" },
];
export const injuryReasonList = [
  { name: "причина травми", value: "падіння та удару до підлоги" },
  { name: "удар до гойдалки", value: "удару до гойдалки" },
  { name: "удар до краю мебелі", value: "удару до краю мебелі" },
  {
    name: "падіння та удар до підлоги",
    value: "падіння та удару до підлоги",
  },
  { name: "зіткнення з іншою дитиною", value: "зіткнення з іншою дитиною" },
];
export const woundSurfaceList = [
  { value: "", name: "поверхня" },
  { value: "шкіра", name: "шкіра" },
  { value: "слизова оболонка", name: "слизова оболонка" },
  { value: "червона облямівка", name: "червона облямівка" },
];
export const woundTypeList = [
  { value: "", name: "вид рани" },
  { value: "розсічена", name: "розсічена" },
  { value: "рвана", name: "рвана" },
  { value: "різана", name: "різана" },
  { value: "кусано-рвана", name: "кусано-рвана" },
  { value: "забійно-рвана", name: "забійно-рвана" },
  { value: "скальповано-рвана", name: "скальповано-рвана" },
  { value: "рублена", name: "рублена" },
  { value: "колота", name: "колота" },
  { value: "скальпована", name: "скальпована" },
];
export const woundFormList = [
  { name: "неправильна форма", value: "неправильної" },
  { name: "лінійна", value: "лінійної" },
  { name: "веретеноподібна", value: "веретеноподібної" },
  { name: "серпоподібна", value: "серпоподібної" },
  { name: "зигзагоподібна", value: "зигзагоподібної" },
  { name: "округла", value: "округлої" },
  { name: "ромбоподібна", value: "ромбоподібної" },
];
export const woundLedgesList = [
  { value: "Краї рани рівні", name: "краї рівні" },
  { value: "Краї рани нерівні", name: "краї нерівні" },
];
export const woundBleedingList = [
  { value: "незначна", name: "кровоточивість незначна" },
  { value: "значна", name: "кровоточивість значна" },
  { value: "відсутня", name: "кровоточивість відсутня" },
];
export const woundAliensList = [
  {
    value: "Візуально сторонніх тіл в рані не виявлено",
    name: "забруднення відсутнє",
  },
  { value: "Рана забруднена брудом", name: "брудом" },
  { value: "Рана забруднена гнійними масами", name: "гнійними масами" },
  { value: "Рана забруднена землею", name: "землею" },
  { value: "Рана забруднена піском", name: "піском" },
  { value: "Рана забруднена дрібним камінням", name: "дрібним камінням" },
  { value: "Рана забруднена залишками фарби", name: "залишками фарби" },
];
export const sutureTypeList = [
  { value: "вузловими швами", name: "вузлові шви" },
  { value: "матрацними швами", name: "матрацні" },
  { value: "внутрішньошкірним безперервним швом", name: "внутрішньошкірний" },
];
export const sutureMaterialList = [
  { value: "PDS ", name: "PDS" },
  { value: "Vicryl ", name: "Vicryl" },
  { value: "Maxon", name: "Maxon" },
  { value: "Monoplus", name: "Monoplus" },
  { value: "Novosyn", name: "Novosyn" },
];
export const sutureSizeList = [
  { value: "5-0 ", name: "5-0" },
  { value: "3-0 ", name: "3-0" },
  { value: "4-0 ", name: "4-0" },
  { value: "6-0 ", name: "6-0" },
];
export const toothList = [
  "",
  55,
  54,
  53,
  52,
  51,
  61,
  62,
  63,
  64,
  65,
  75,
  74,
  73,
  72,
  71,
  81,
  82,
  83,
  84,
  85,
  16,
  26,
  36,
  46,
  11,
  12,
  13,
  14,
  15,
  17,
  18,
  21,
  22,
  23,
  24,
  25,
  27,
  28,
  31,
  32,
  33,
  34,
  35,
  37,
  38,
  41,
  42,
  43,
  44,
  45,
  47,
  48,
];
export const retention1List = ["", 18];
export const retention2List = ["", 28];
export const retention3List = ["", 38];
export const retention4List = ["", 48];
export const morbiList = [
  { name: "", value: "" },
  { name: "г.глибокий", value: "acuteDeep" },
  { name: "г.середній", value: "acuteMiddle" },
  { name: "г.поверх.", value: "acuteFacial" },
  { name: "х.глибокий", value: "chronicDeep" },
  { name: "х.середній", value: "chronicMiddle" },
  { name: "х.поверх.", value: "chronicFacial" },
  { name: "пульпіт", value: "pulpit" },
  { name: "періодонтит", value: "periodontit" },
];
export const abscessTypeList = [
  { name: "абсцес", value: "абсцес" },
  { name: "флегмона", value: "флегмона" },
  { name: "фурункул", value: "фурункул" },
  { name: "лімфаденіт", value: "лімфаденіт" },
  { name: "гнійний лімфаденіт", value: "гнійний лімфаденіт" },
  { name: "сіалоаденіт", value: "сіалоаденіт" },
];

export const abscessAreaList = [
  { value: "", name: "ділянка" },
  { value: "підщелепної ділянки", name: "підщелепна" },
  { value: "субментальної ділянки", name: "субментальна" },
  {
    value: "в ділянці сонного трикутника",
    name: "ділянка сонного трикутника",
  },
  { value: "привушної ділянки", name: "привушна" },
  { value: "підборідної ділянки", name: "підборідна" },
  { value: "щічної ділянки", name: "щічна" },
  { value: "нижньої губи", name: "нижня губа" },
  { value: "верхньої губи", name: "верхня губа" },
  { value: "підочної ділянки", name: "підочна" },
  { value: "виличної ділянки", name: "вилична" },
  { value: "лобної ділянки", name: "лобна" },
  { value: "позадувушної ділянки", name: "позадувушна" },
  { value: "дна порожнини рота", name: "дно порожнини рота" },
  { value: "твердого піднебіння", name: "тверде піднебіння" },
  { value: "щелепно-язикового жолобка", name: "щелепно-язиковий жолобок" },
  { value: "крило-щелепного простору", name: "крило-щелепний простір" },
];
export const abscessReasonList = [
  { name: "причина", value: "" },
  { name: "лімфовузол", value: "лімфовузол" },
  { name: "зуб", value: "зуб" },
  { name: "відсутня", value: "" },
];
export const cystTypeList = [
  { name: "вид кісти", value: "" },
  { name: "фолікулярна", value: "фолікулярна кіста" },
  { name: "радикулярна", value: "радикулярна кіста" },
  { name: "кістогранульома", value: "апікальна кістогранульома" },
];
export const rootSealedList = [
  { name: "канал зуба", value: "" },
  { name: "запломбовано", value: "rootSealed" },
  { name: "незпломбовано", value: "rootSealedNot" },
];
export const neoplasmTypeList = [
  { name: "новоутвір", value: "новоутвір" },
  { name: "папілома", value: "папілома" },
  { name: "ретенційна кіста", value: "ретенційна кіста" },
  { name: "фіброма", value: "фіброма" },
  { name: "атерома", value: "атерома" },
];
export const growthRateList = [
  { name: "темп росту", value: "" },
  { name: "швидко", value: "швидко" },
  { name: "поволі", value: "поволі" },
  {
    name: "спочатку швидко, потім поволі",
    value: "спочатку швидко, потім поволі",
  },
  {
    name: "спочатку поволі, потім швидко",
    value: "спочатку поволі, потім швидко",
  },
];
export const neoplasmFormList = [
  { name: "форма", value: "" },
  { name: "кругла", value: "круглої" },
  { name: "овальна", value: "овальної" },
  { name: "циліндрична", value: "циліндричної" },
  { name: "неправильна", value: "неправильної" },
  { name: "конусоподібна", value: "конусоподібної" },
];
export const neoplasmBorderList = [
  { name: "чіткі межі", value: "з чіткими межами" },
  { name: "розмиті межі", value: "з розмитими межами" },
  { name: "нерівні краї", value: "з нерівними краями" },
];
export const neoplasmMovabilityList = [
  { name: "рухомий", value: "рухомий" },
  { name: "нерухомий", value: "нерухомий" },
  { name: "обмежено рухомий", value: "обмежено рухомий" },
];
export const neoplasmPoignancyList = [
  { name: "безболісний", value: "безболісний" },
  { name: "болісний", value: "болісний" },
];
export const neoplasmConsistanceList = [
  { name: "щільна", value: "щільної" },
  { name: "м'яка", value: "м'якої" },
  { name: "еластична", value: "еластичної" },
  { name: "щільно-еластична", value: "щільно-еластичної" },
  { name: "мяко-еластична", value: "м'яко-еластичної" },
  { name: "тістоподібна", value: "тістоподібної" },
];
export const neoplasmSurfaceList = [
  { value: "шкіра", name: "шкіра" },
  { value: "слизова оболонка", name: "слизова оболонка" },
  { value: "червона облямівка", name: "червона облямівка" },
];
export const neoplasmColorList = [
  { name: "забарвлення", value: "" },
  { name: "червоний", value: "червоного" },
  { name: "темно-бурий", value: "темно-бурого" },
  { name: "синюшний", value: "синюшного" },
  { name: "темно-коричневий", value: "темно-коричневого" },
  { name: "блідо-рожевий", value: "блідо-рожевого" },
];
export const uColorList = [
  "жовта",
  "світло-жовта",
  "темно-жовта",
  "помаранчева",
  "бліда",
  "рожева",
  "темно-рожева",
];
export const uOpacityList = ["прозора", "непрозора", "слабко-мутна", "мутна"];
export const enterobiozList = [
  { name: "не визначався", value: "не визначався" },
  {
    name: "яйця гостриків не виявлено",
    value: " яйця гостриків не виявлено",
  },
  { name: "яйця гостриків виявлено", value: " яйця гостриків виявлено" },
];
export const dungList = [
  { name: "не визначався", value: "не визначався" },
  {
    name: "яйця гельмінтів не виявлено",
    value: "яйця гельмінтів не виявлено",
  },
  { name: "яйця гельмінтів виявлено", value: "яйця гельмінтів виявлено" },
];
export const bloodGroupList = [
  { name: "група крові", value: "" },
  { name: "I(0)", value: "I(0)" },
  { name: "II(A)", value: "II(A)" },
  { name: "III(B)", value: "III(B)" },
  { name: "IV(AB)", value: "IV(AB)" },
];
export const rezusFactorList = [
  { name: "резус-фактор", value: "" },
  { name: "+", value: "+" },
  { name: "-", value: "-" },
];

export const restMaterialList = [
  { value: "", name: "пломбувальний матеріал" },
  { value: '"Latelux"', name: "Latelux" },
  { value: '"Charisma"', name: "Charisma" },
  { value: '"Filtek"', name: "Filtek" },
  { value: '"Spectrum"', name: "Spectrum" },
  { value: '"Dyract XP"', name: "Dyract XP" },
  { value: "Gradia", name: "Gradia" },
];
export const editTypeList = [
  { value: "", name: "оберіть частину" },
  { value: "name", name: "ПІБ пацієнта" },
  { value: "cardNumber", name: "номер мед.карти" },
  { value: "reviewDate", name: "дата огляду" },
  { value: "doctor", name: "особистий лікар" },
  { value: "complaintsContent", name: "скарги" },
  { value: "anamnesisMorbiContent", name: "анамнез хвороби" },
  { value: "anamnesisVitaeContent", name: "анамнез життя" },
  { value: "statusPraesensContent", name: "об'єктивний стан" },
  { value: "statusLocalisContent", name: "стан ЩЛД" },
  { value: "diagnosis", name: "діагноз" },
  { value: "examinations", name: "перелік обстежень" },
  { value: "protocolNumber", name: "номер протоколу" },
  { value: "operationName", name: "назва операції" },
  { value: "operationDate", name: "дата операції" },
  { value: "operationTime", name: "час операції" },
  { value: "operationContent", name: "протокол операції" },
  { value: "drugName1", name: "антибіотик" },
  { value: "drugName2", name: "протизапальний" },
  { value: "drugName3", name: "пробіотик" },
  { value: "drugName4", name: "антигістамінний" },
  { value: "drugName5", name: "інші призначення" },
  { value: "appointment", name: "дод. призначення 1" },
  { value: "appointment2", name: "дод. призначення 2" },
  { value: "appointment3", name: "дод. призначення 3" },
  { value: "recommendations", name: "рекомендації" },
  { value: "shortStatusContent", name: "обгрунтування діагнозу" },
  { value: "secondOperation", name: "друга операція" },
  { value: "finalDiagnosis", name: "заключний діагноз" },
  { value: "otherExaminations", name: "інші обстеження" },
];
export const bloodTestExponents = [
  "Hb, г/л",
  "Er, T/л	",
  "КП",
  "Leu, Г/л",
  "пал, %",
  "сегм, %",
  "еоз, %",
  "лімф, %",
  "мон, %",
  "ШОЕ, мм/год",
];
export const urineTestExponents = [
  "Колір",
  "Прозорість",
  "Пит. вага",
  "pH",
  "Білок",
  "Лейкоцити",
  "Плоский епіт.",
  "Перехідний епіт.",
  "Інше",
];
export const initialExaminations = 'Загальний аналіз крові,Загальний аналіз сечі,Глюкоза крові';
export const baseIndications = [
  "Згоду батьків до операції отримано.",
  "Протипоказання до операції відсутні.",
];
export const bilateralismList = [
  { name: "однобічний", value: false },
  { name: "двобічний", value: true },
];
export const fractureLowerAreaList = [
  { name: "ділянка перелому", value: "" },
  { name: "центральний", value: "центральний" },
  { name: "ментальний", value: "ментальний" },
  { name: "тіла в ділянці молярів", value: "тіла" },
  { name: "кутовий", value: "кутовий" },
  { name: "гілки", value: "гілки" },
  { name: "суглобового відростка", value: "суглобового відростка" },
  { name: "переломо-вивих", value: "переломо-вивих" },
];
export const fractureBiasList = ["без зміщення", "зі зміщенням"];
export const fractureDirectionList = ["лінійний", "косий", "зигзагоподібний"];
export const fractureLowerTeethList = [
  ``,
  38,
  37,
  36,
  35,
  34,
  33,
  32,
  31,
  41,
  42,
  43,
  44,
  45,
  46,
  47,
  48,
  75,
  74,
  73,
  72,
  71,
  81,
  82,
  83,
  84,
  85,
];
export const teethInFractureLineList = [
  { name: "зуби інтактні", value: "" },
  { name: "лінія через зуб", value: "through" },
  { name: "лінія між зубами", value: "between" },
];
export const fractureStepList = [
  { name: "симптом сходинки", value: "неінформативний із-за набряку тканин" },
  { name: "відсутній", value: "відсутній" },
  { name: "позитивний", value: "позитивний" },
  { name: "неінформативний", value: "неінформативний із-за набряку тканин" },
];
export const xRayList = ["рентген", "КТ"];
export const continuityList = [
  { name: "безперерв. нижнього краю", value: "не порушена" },
  { name: "не порушена", value: "не порушена" },
  { name: "порушена", value: "порушена" },
];
export const fractureOperationTypeList = [
  { name: "тип операції", value: "" },
  { name: "гвинтами", value: "гвинтами" },
  { name: "шинами Васильєва", value: "шинами Васильєва" },
  { name: "по Айві", value: "по Айві" },
  { name: "остеосинтез", value: "остеосинтез" },
];
export const dlTypeList = [
  { name: "тип вивиху", value: "" },
  { name: "повний", value: "повний" },
  { name: "неповний", value: "неповний" },
  { name: "вколочений", value: "вколочений" },
];
export const dlOperationTypeList = [
  { name: "тип операції", value: "" },
  { name: "видалення", value: "extraction" },
  { name: "ушивання", value: "suturing" },
  { name: "шина-скоба", value: "splinting" },
  { name: "шина-ретейнер", value: "retainer" },
];
export const dlToothList = toothList.sort();
export const dlDirectionList = [
  { name: "зміщення", value: "" },
  { name: "вниз", value: "вниз" },
  { name: "вгору", value: "вгору" },
];
export const overCompleteLocationList = [
  { name: "положення", value: "" },
  { name: "вестибулярне", value: "vestibular" },
  { name: "піднебінне", value: "palatine" },
  { name: "язикове", value: "lingual" },
];
export const xRayList2 = [
  { name: "оглядова Rg", value: "Rg" },
  { name: "КТ", value: "СT" },
];
export const overCompleteFormList = [
  { name: "форма", value: "" },
  { name: "конусоподібна", value: "конусоподібної" },
  { name: "шилоподібна", value: "шилоподібної" },
  { name: "анатомічна", value: "анатомічної" },
];
export const overCompleteAmountList = [
  { name: "кількість зубів", value: 2 },
  { name: 1, value: 1 },
  { name: 2, value: 2 },
];
export const days = [
  "",
  "1 день",
  "2 дні",
  "3 дні",
  "4 дні",
  "5 днів",
  "6 днів",
  "7 днів",
  "8 днів",
  "9 днів",
  "10 днів",
];

export const maleNames = [
  "Аркадій",
  "Антон",
  "Артур",
  "Арсен",
  "Анатолій",
  "Семен",
  "Олег",
  "Василь",
  "Степан",
  "Михайло",
  "Мирон",
  "Мирослав",
  "Роман",
  "Тарас",
  "Святослав",
  "В'ячеслав",
  "Ярослав",
  "Володимир",
  "Петро",
  "Павло",
  "Гнат",
  "Борис",
  "Ростислав",
  "Станіслав",
  "Семен",
  "Тимур",
  "Лука",
  "Тимофій",
  "Тадей",
  "Пилип",
  "Адріан",
  "Андрій",
  "Валерій",
  "Едуард",
  "Віталій",
  "Віктор",
  "Григорій",
  "Максим",
  "Констянтин",
  "Ілля",
  "Любомир",
  "Руслан",
  "Рустам",
  "Назар",
  "Назарій",
  "Олексій",
  "Захар",
  "Олександр",
  "Іван",
  "Ігор",
  "Зіновій",
  "Артем",
  "Даніель",
  "Данило",
  "Артур",
  "Денис",
  "Владислав",
  "Вадим",
  "Юрій",
  "Владлен",
  "Матвій",
  "Устим",
  "Йосип",
  "Осип",
  "Орест",
  "Генадій",
  "Адам",
  "Кирило",
  "Гліб",
  "Богдан",
  "Євген",
  "Марк",
  "Марко",
  "Маркіян",
  "Адріян",
  "Олесь",
  "Сергій",
  "Альберт",
  "Валентин",
  "Влад",
  "Всеволод",
  "Георгій",
  "Гордій",
  "Дем'ян",
  "Давид",
  "Дмитро",
  "Єгор",
  "Зиновій",
  "Зорян",
  "Леонід",
  "Лев",
  "Мартин",
  "Мар'ян",
  "Нестор",
  "Омелян",
  "Остап",
  "Горден",
  "Северин",
  "Федір",
  "Юлій",
  "Юхим",
  "Юліан",
  "Ян",
  "Микола",
  "Ренат",
  "Самуїл",
  "Даниїл",

  // "",
];

export const upperConstantTeeth = [
  18, 17, 16, 15, 14, 13, 12, 11, 21, 22, 23, 24, 25, 26, 27, 28,
];
export const lowerConstantTeeth = [
  48, 47, 46, 45, 44, 43, 42, 41, 31, 32, 33, 34, 35, 36, 37, 38,
];
export const upperTemporaryTeeth = [55, 54, 53, 52, 51, 61, 62, 63, 64, 65];
export const lowerTemporaryTeeth = [85, 84, 83, 82, 81, 71, 72, 73, 74, 75];
export const letters = [
  { name: "Літера...", value: "" },
  { name: "А", value: "А" },
  { name: "Б", value: "Б" },
  { name: "В", value: "В" },
  { name: "Г", value: "Г" },
  { name: "Д", value: "Д" },
  { name: "Е", value: "Е" },
  { name: "Є", value: "Є" },
  { name: "Ж", value: "Ж" },
  { name: "З", value: "З" },
];
export const townList = [
  {
    value: "",
    name: "оберіть місто",
    streets: [{ value: "", name: "оберіть вулицю" }],
  },
  {
    value: "Тернопіль",
    name: "Тернопіль",
    streets: [
      { name: "15 квітня" },
      { name: "Багата" },
      { name: "Балея" },
      { name: "Барвінських" },
      { name: "Безкоровайного" },
      { name: "Бенцаля" },
      { name: "Бережанська" },
      { name: "Березова" },
      { name: "Білецька" },
      { name: "Білогірська" },
      { name: "Блажкевич" },
      { name: "Богуна" },
      { name: "Бойківська" },
      { name: "Бойчуків" },
      { name: "Болюха" },
      { name: "Бордуляка" },
      { name: "Батьківського" },
      { name: "Броварна" },
      { name: "Бродівська" },
      { name: "Бриковича" },
      { name: "Брюкнера" },
      { name: "Будзиновського" },
      { name: "Будного" },
      { name: "Вагилевича" },
      { name: "Валова" },
      { name: "Вербицького" },
      { name: "Вербова" },
      { name: "Вертепна" },
      { name: "Весела" },
      { name: "Виговського" },
      { name: "Винниченка" },
      { name: "Вишнева" },
      { name: "Вишневецького" },
      { name: "Віконської" },
      { name: "Вільхова" },
      { name: "Вітовського" },
      { name: "Волинська" },
      { name: "Володимира Великого" },
      { name: "Вояків дивізії `Галичина`" },
      { name: "Гавдиди" },
      { name: "Гайова" },
      { name: "Гайова-бічна" },
      { name: "Галицька" },
      { name: "Галицького" },
      { name: "Героїв Крут" },
      { name: "Героїв Чорнобиля" },
      { name: "Гірняка" },
      { name: "Глибока" },
      { name: "Глибока Долина" },
      { name: "Глибочанська" },
      { name: "Глиняна" },
      { name: "Гнатюка" },
      { name: "Гніздовського" },
      { name: "Гоголя" },
      { name: "Головацького" },
      { name: "Голубовича" },
      { name: "Горбачевського" },
      { name: "Городна" },
      { name: "Грабовського" },
      { name: "Гребінки" },
      { name: "Громницького" },
      { name: "Грушевського" },
      { name: "Гузара" },
      { name: "Гуцульська" },
      { name: "Ґжицького" },
      { name: "Ґріґа" },
      { name: "Далека" },
      { name: "Деповського" },
      { name: "Дівоча" },
      { name: "Ділова" },
      { name: "Дністрянського" },
      { name: "Довбуша" },
      { name: "Довженка" },
      { name: "Доли" },
      { name: "Дорошенка" },
      { name: "Драгоманова" },
      { name: "Дружби" },
      { name: "Дубовецька" },
      { name: "Енергетична" },
      { name: "За Рудкою" },
      { name: "Загребельна" },
      { name: "Заміська" },
      { name: "Замкова" },
      { name: "Замонастирська" },
      { name: "Зарічна" },
      { name: "Захисників України" },
      { name: "Збаразька" },
      { name: "Зелена" },
      { name: "Злуки" },
      { name: "Золотогірська" },
      { name: "Іллєнка" },
      { name: "Калинова" },
      { name: "Камінна" },
      { name: "Канадська" },
      { name: "Кармелюка" },
      { name: "Карпенка" },
      { name: "Качали" },
      { name: "Квітки Цісик" },
      { name: "Квітова" },
      { name: "Київська" },
      { name: "Клима Савура" },
      { name: "Клінічна" },
      { name: "Козацька" },
      { name: "Коллонтая" },
      { name: "Коновальця" },
      { name: "Коперника" },
      { name: "Корольова" },
      { name: "Котляревського" },
      { name: "Коцюбинського" },
      { name: "Кривоноса" },
      { name: "Крушельницької" },
      { name: "Куліша" },
      { name: "Кульчинської" },
      { name: "Купчинського" },
      { name: "Куток" },
      { name: "Лемківська" },
      { name: "Леонтовича" },
      { name: "Лепкого" },
      { name: "Лесі Українки" },
      { name: "Леся Курбаса" },
      { name: "Липницького" },
      { name: "Липова" },
      { name: "Лисенка" },
      { name: "Листопадова" },
      { name: "Лісова" },
      { name: "Лозовецька" },
      { name: "Лукіяновича" },
      { name: "Лучаковського" },
      { name: "Львівська" },
      { name: "Мазепи" },
      { name: "Малишка" },
      { name: "Манастирського" },
      { name: "Марка Вовчка" },
      { name: "Медобірна" },
      { name: "Медова" },
      { name: "Межова" },
      { name: "Мєшковського" },
      { name: "Микулинецька" },
      { name: "Микулинецька-бічна" },
      { name: "Мирна" },
      { name: "Миру" },
      { name: "Михалевича" },
      { name: "Молодіжна" },
      { name: "Монюшка" },
      { name: "Морозенка" },
      { name: "Мостова" },
      { name: "Мостова-бічна" },
      { name: "Мстислава" },
      { name: "Набережна" },
      { name: "Над Ставом" },
      { name: "Над Яром" },
      { name: "Надзбручанська" },
      { name: "Наливайка" },
      { name: "Нечая" },
      { name: "Низинна" },
      { name: "Нова" },
      { name: "Новий Світ" },
      { name: "Новий Світ-бічна" },
      { name: "Новосонячна" },
      { name: "Об'їзна" },
      { name: "Оболоня" },
      { name: "Овочева" },
      { name: "Опільського" },
      { name: "Орлика" },
      { name: "Острозького" },
      { name: "Паращука" },
      { name: "Паркова" },
      { name: "Пелиха" },
      { name: "Перля" },
      { name: "Петлюри" },
      { name: "Петриківська" },
      { name: "Петрушевича" },
      { name: "Пирогова" },
      { name: "Південна" },
      { name: "Північна" },
      { name: "Підгірна" },
      { name: "Підгородня" },
      { name: "Підкови" },
      { name: "Піскова" },
      { name: "Подільська" },
      { name: "Покрови" },
      { name: "Поліська" },
      { name: "Польова" },
      { name: "Польового" },
      { name: "Приміська" },
      { name: "Проектна" },
      { name: "Промислова" },
      { name: "Просвіти" },
      { name: "Протасевича" },
      { name: "Пулюя" },
      { name: "Рєпіна" },
      { name: "Рівна" },
      { name: "Рудницького" },
      { name: "Руставелі" },
      { name: "Руська" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Самчука" },
      { name: "Сахарова" },
      { name: "Симоненка" },
      { name: "Сімовича" },
      { name: "Сірка" },
      { name: "Січинського" },
      { name: "Січових Стрільців" },
      { name: "Слівенська" },
      { name: "Сліпого" },
      { name: "Словацького" },
      { name: "Смакули" },
      { name: "Сонячна" },
      { name: "Спадиста" },
      { name: "Спортивна" },
      { name: "Стадникової" },
      { name: "Старий Поділ" },
      { name: "Старий Ринок" },
      { name: "Степана Бандери" },
      { name: "Степова" },
      { name: "Стефаника" },
      { name: "Стецька" },
      { name: "Стрімка" },
      { name: "Студинського" },
      { name: "Стуса" },
      { name: "Тарнавського" },
      { name: "Татарська" },
      { name: "Тбіліська" },
      { name: "Текстильна" },
      { name: "Теліги" },
      { name: "Тернопільська" },
      { name: "Тиха" },
      { name: "Тісна" },
      { name: "Торговиця" },
      { name: "Транспортна" },
      { name: "Тролейбусна" },
      { name: "Тютюнника" },
      { name: "Урожайна" },
      { name: "Фабрична" },
      { name: "Федьковича" },
      { name: "Фестивальна" },
      { name: "Франка" },
      { name: "Хліборобна" },
      { name: "Хмельницького" },
      { name: "Хутірська" },
      { name: "Циганська" },
      { name: "Чайковського" },
      { name: "Чарнецького" },
      { name: "Чернівецька" },
      { name: "Чорновола" },
      { name: "Чубинського" },
      { name: "Чумацька" },
      { name: "Шагайди" },
      { name: "Шашкевича" },
      { name: "Шевченка" },
      { name: "Шептицького" },
      { name: "Шкільна" },
      { name: "Шопена" },
      { name: "Шпитальна" },
      { name: "Шухевича" },
      { name: "Юності" },
      { name: "Юрчака" },
      { name: "Яблунева" },
      { name: "Яреми" },
      { name: "Яремчука" },
      { name: "Ярмуша" },
      { name: "Ясна" },
    ],
  },
  {
    value: "Бережани",
    name: "Бережани",
    streets: [
      { name: "1 травня" },
      { name: "8 березня" },
      { name: "Адамівська" },
      { name: "Бандери" },
      { name: "Банкова" },
      { name: "Богуна" },
      { name: "Бояна" },
      { name: "Браїлка" },
      { name: "Валова" },
      { name: "Ватутіна" },
      { name: "Верхня" },
      { name: "Вільховецька" },
      { name: "Вірменська" },
      { name: "Володимирська" },
      { name: "Гагаріна" },
      { name: "Гайова" },
      { name: "Галана" },
      { name: "Галицького" },
      { name: "Гімназійна" },
      { name: "Гоголя" },
      { name: "Грушевського" },
      { name: "Депутатська" },
      { name: "Джерельна" },
      { name: "Довженка" },
      { name: "Дружби" },
      { name: "Загороди" },
      { name: "Залісся" },
      { name: "Замкова" },
      { name: "Замость" },
      { name: "Зарайський потік" },
      { name: "Зарічна" },
      { name: "Захарка" },
      { name: "Західна" },
      { name: "Збічна" },
      { name: "Зелена" },
      { name: "Золочівська" },
      { name: "Зоряна" },
      { name: "Інститутська" },
      { name: "Івана Франка" },
      { name: "Кармелюка" },
      { name: "Кобилянської" },
      { name: "Ковпака" },
      { name: "Ковшевича" },
      { name: "Козацька" },
      { name: "Комарова" },
      { name: "Коперника" },
      { name: "Корольова" },
      { name: "Котляревського" },
      { name: "Коцюбинського" },
      { name: "Кривоноса" },
      { name: "Крута" },
      { name: "Крушельницької" },
      { name: "Кульчицької" },
      { name: "Кушніра" },
      { name: "Левицького" },
      { name: "Лепких" },
      { name: "Лесі Українки" },
      { name: "Лисенка" },
      { name: "Львівська" },
      { name: "Мазепи" },
      { name: "Маковея" },
      { name: "Медова" },
      { name: "Мечнікова" },
      { name: "Мирного" },
      { name: "Миру" },
      { name: "Міцкевича" },
      { name: "Молодіжна" },
      { name: "Монастирського" },
      { name: "Набережна" },
      { name: "Нахімова" },
      { name: "Незалежності" },
      { name: "Пирогова" },
      { name: "Підгірна" },
      { name: "Підкови" },
      { name: "Підлісна" },
      { name: "Площа Ринок" },
      { name: "Привокзальна" },
      { name: "Просвіти" },
      { name: "Пушкіна" },
      { name: "Репіна" },
      { name: "Рильського" },
      { name: "Ринок" },
      { name: "Рогатинська" },
      { name: "Родини Старухів" },
      { name: "Руриско" },
      { name: "Руська" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Старе Місто" },
      { name: "Степана Бандери" },
      { name: "Стефаника" },
      { name: "Твердохліба" },
      { name: "Тепла" },
      { name: "Тернопільська" },
      { name: "Тиха" },
      { name: "Тичини" },
      { name: "Топольна" },
      { name: "Хатки" },
      { name: "Хмельницького" },
      { name: "Цегельна" },
      { name: "Чайковського" },
      { name: "Червона" },
      { name: "Чорновола" },
      { name: "Шашкевича" },
      { name: "Шевченка" },
      { name: "Шептицького" },
    ],
  },
  {
    value: "Борщів",
    name: "Борщів",
    streets: [
      { name: "Бажана" },
      { name: "Базарна" },
      { name: "Богдана Хмельницького" },
      { name: "Богуна" },
      { name: "Борундяка" },
      { name: "Верхрадського" },
      { name: "Винниченка" },
      { name: "Відродження" },
      { name: "Вовківська" },
      { name: "Володимира Великого" },
      { name: "Володимира Івасюка" },
      { name: "Галицька" },
      { name: "Галичко" },
      { name: "Гетьмана Івана Мазепи" },
      { name: "Гніздовського" },
      { name: "Григорія Мазепи" },
      { name: "Грушевського" },
      { name: "Данила Галицького" },
      { name: "Довбуша" },
      { name: "Долина" },
      { name: "Дружня" },
      { name: "Заводська" },
      { name: "Замкова" },
      { name: "Зелена" },
      { name: "Івана Франка" },
      { name: "Йосипа Сліпого" },
      { name: "Кармелюка" },
      { name: "Катерини Попович" },
      { name: "Катюкова" },
      { name: "Козацька" },
      { name: "Колянківського" },
      { name: "Коновальця" },
      { name: "Котляревського" },
      { name: "Кошового" },
      { name: "Кривоноса" },
      { name: "Крушельницької" },
      { name: "Кут" },
      { name: "Левадна" },
      { name: "Лепкого" },
      { name: "Лесі Українки" },
      { name: "Леся Курбаса" },
      { name: "Лисенка" },
      { name: "Лісова" },
      { name: "Лозова" },
      { name: "Маковея" },
      { name: "Медова" },
      { name: "Нагірна" },
      { name: "Наливайка" },
      { name: "Незалежності" },
      { name: "Нічлава" },
      { name: "Нова" },
      { name: "Об'їздова" },
      { name: "Олімпійська" },
      { name: "Підвальна" },
      { name: "Підлісна" },
      { name: "Польова" },
      { name: "Пулюя" },
      { name: "Рубчакової" },
      { name: "Русенка" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Симона Петлюри" },
      { name: "Січових Стрільців" },
      { name: "Сковороди" },
      { name: "Староборщівська" },
      { name: "Степана Бандери" },
      { name: "Стуса" },
      { name: "Тичини" },
      { name: "Хмельницького" },
      { name: "Черемшини" },
      { name: "Чернецького" },
      { name: "Чорновола" },
      { name: "Чубинського" },
      { name: "Шафранюка" },
      { name: "Шевченка" },
      { name: "Шкільна" },
      { name: "Шухевича" },
      { name: "Ярослава Кондри" },
      { name: "Ярослава Мудрого" },
    ],
  },
  {
    value: "Бучач",
    name: "Бучач",
    streets: [
      { name: "1 грудня" },
      { name: "Агнона" },
      { name: "Бариська" },
      { name: "Баштова" },
      { name: "Бічна Бариська" },
      { name: "Бічна Богдана Хмельницького" },
      { name: "Бічна Вузька" },
      { name: "Бічна Замкова" },
      { name: "Бічна Лисенка" },
      { name: "Бічна Садова" },
      { name: "Бічна Цегельна" },
      { name: "Богдана Хмельницького" },
      { name: "Вербова" },
      { name: "Весела" },
      { name: "Винниченка" },
      { name: "Вишнева" },
      { name: "Водна" },
      { name: "Василя Стуса" },
      { name: "Вузька" },
      { name: "Галицька" },
      { name: "Гоголя" },
      { name: "Довбуша" },
      { name: "Дорошенка" },
      { name: "Заводська" },
      { name: "Залізнична" },
      { name: "Замкова" },
      { name: "Зелена" },
      { name: "Івана Франка" },
      { name: "Калинова" },
      { name: "Кам'яна" },
      { name: "Квітнева" },
      { name: "Кобилянської" },
      { name: "Ковпака" },
      { name: "Космонавтів" },
      { name: "Коцюбинського" },
      { name: "Коцюбинської" },
      { name: "Кринична" },
      { name: "Куліша" },
      { name: "Курбаса" },
      { name: "Лепкого" },
      { name: "Лесі Українки" },
      { name: "Лисенка" },
      { name: "Миру" },
      { name: "Міцкевича" },
      { name: "Млинарська" },
      { name: "Морозенка" },
      { name: "Мулярська" },
      { name: "Некрасова" },
      { name: "Підгаєцька" },
      { name: "Польова" },
      { name: "Просвіти" },
      { name: "Пушкіна" },
      { name: "Рильська" },
      { name: "Руська" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Степана Бандери" },
      { name: "Святого Миколая" },
      { name: "Середня" },
      { name: "Січових Стрільців" },
      { name: "Соломії Крушельницької" },
      { name: "Сонячна" },
      { name: "Сонячна бічна" },
      { name: "Сосюри" },
      { name: "Спадова" },
      { name: "Стефаника" },
      { name: "Стрипна" },
      { name: "Тичини" },
      { name: "Торгова" },
      { name: "Травнева" },
      { name: "Хвильового" },
      { name: "Цегельна" },
      { name: "Чайківського" },
      { name: "Шевченка" },
      { name: "Шкільна" },
      { name: "Шухевича" },
      { name: "Яворницького" },
    ],
  },
  {
    value: "Заліщики, ",
    name: "Заліщики",
    streets: [
      { name: "40 років Перемоги" },
      { name: "Богуна" },
      { name: "Богдана Хмельницького" },
      { name: "Весняна" },
      { name: "Володимира Великого" },
      { name: "Гайворонська" },
      { name: "Гайворонського" },
      { name: "Галана" },
      { name: "Гонти" },
      { name: "Гранична" },
      { name: "Грушевського" },
      { name: "Данила Галицького" },
      { name: "Дворище" },
      { name: "Джерельна" },
      { name: "Драгоманова" },
      { name: "Залізнична" },
      { name: "Заньковецької" },
      { name: "Зелена" },
      { name: "Зеленогайська" },
      { name: "Івана Франка" },
      { name: "Йосипа Сліпого" },
      { name: "Козацька" },
      { name: "Коновальця" },
      { name: "Кооперативна" },
      { name: "Котляревського" },
      { name: "Кропивницького" },
      { name: "Кустря" },
      { name: "Лепкого" },
      { name: "Лисенка" },
      { name: "Мазепи" },
      { name: "Маковея" },
      { name: "Марка Вовчка" },
      { name: "Миколаївка" },
      { name: "Миру" },
      { name: "Наддністрянська" },
      { name: "Надсекретна" },
      { name: "Наливайка" },
      { name: "Незалежності" },
      { name: "Нова" },
      { name: "Ольги Кобилянської" },
      { name: "Ольжича" },
      { name: "Подільська" },
      { name: "Полуботка" },
      { name: "Польова" },
      { name: "Поповича" },
      { name: "Рибацька" },
      { name: "Сагайдачного" },
      { name: "Симоненка" },
      { name: "Січових Стрільців" },
      { name: "Соломії Крушельницької" },
      { name: "Степана Бандери" },
      { name: "Стефаника" },
      { name: "Стуса" },
      { name: "Тирешкова" },
      { name: "Устименка" },
      { name: "Ущилівська" },
      { name: "Філатова" },
      { name: "Чорновола" },
      { name: "Чубинського" },
      { name: "Шашкевича" },
      { name: "Швайська" },
      { name: "Шевченка" },
      { name: "Шептицького" },
      { name: "Шкільна" },
      { name: "Шухевича" },
    ],
  },
  {
    value: "Збараж",
    name: "Збараж",
    streets: [
      { name: "22 січня" },
      { name: "50-річчя УПА" },
      { name: "780-річчя Збаража" },
      { name: "Айвазовська" },
      { name: "Базаринецька" },
      { name: "Барвінкова" },
      { name: "Богдана Хмельницького" },
      { name: "Богуславки" },
      { name: "Бульби" },
      { name: "Володимира Великого" },
      { name: "Винника" },
      { name: "Вишнева" },
      { name: "Вишнівецького" },
      { name: "Волинська" },
      { name: "Гайдамацька" },
      { name: "Гвардіціва" },
      { name: "Героїв Крут" },
      { name: "Гоголя" },
      { name: "Гонти" },
      { name: "Горбачевського" },
      { name: "Грушевського" },
      { name: "Данила Галицького" },
      { name: "Довга" },
      { name: "Дорошенка" },
      { name: "Дружби" },
      { name: "Енергетична" },
      { name: "Заводська" },
      { name: "Залізнична" },
      { name: "Замківського" },
      { name: "Заньковецької" },
      { name: "Зелена" },
      { name: "Злуки" },
      { name: "Івана Мазепи" },
      { name: "Івана Франка" },
      { name: "Івана Франка-бічна" },
      { name: "Кантемирівці" },
      { name: "Івана Франка" },
      { name: "Карпенка-Карого" },
      { name: "Клячківського" },
      { name: "Княгині Ольги" },
      { name: "Козубських" },
      { name: "Коперніка" },
      { name: "Корнійчука" },
      { name: "Корольова" },
      { name: "Космонавтів" },
      { name: "Костомарова" },
      { name: "Котляревського" },
      { name: "Коцюбинського" },
      { name: "Кривоноса" },
      { name: "Крип'якевича" },
      { name: "Крушельницької" },
      { name: "Куліша" },
      { name: "Курбаса" },
      { name: "Лесі Українки" },
      { name: "Лепкого" },
      { name: "Лисенка" },
      { name: "Ломоносова" },
      { name: "Лугова" },
      { name: "Львівська" },
      { name: "Майбороди" },
      { name: "Маковея" },
      { name: "Максимівське шосе" },
      { name: "Малюти" },
      { name: "Маневича" },
      { name: "Мирного" },
      { name: "Миру" },
      { name: "Мистецька" },
      { name: "Міцкевича" },
      { name: "Молодіжна" },
      { name: "Морозенка" },
      { name: "Набережна" },
      { name: "Наливайка" },
      { name: "Незалежності" },
      { name: "Нечая" },
      { name: "Низька" },
      { name: "Оболоня" },
      { name: "Ольги Кобилянської" },
      { name: "Осмолинка" },
      { name: "Острозького" },
      { name: "Павленка" },
      { name: "Паркова" },
      { name: "Петлюри" },
      { name: "Підгорева" },
      { name: "Подільська" },
      { name: "Полуботка" },
      { name: "Польова" },
      { name: "Поповича" },
      { name: "Прашка" },
      { name: "Просвіти" },
      { name: "Пушкіна" },
      { name: "Рильського" },
      { name: "Родини Білінських" },
      { name: "Руданського" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Садовського" },
      { name: "Самчука" },
      { name: "Степана Бандери" },
      { name: "Симеренка" },
      { name: "Симоненка" },
      { name: "Сірка" },
      { name: "Січових Стрільців" },
      { name: "Слов'янська" },
      { name: "Сонячна" },
      { name: "Сосюри" },
      { name: "Стефаника" },
      { name: "Стуса" },
      { name: "Технічна" },
      { name: "Тихого" },
      { name: "Тичини" },
      { name: "Тімірязєва" },
      { name: "Травнева" },
      { name: "Тургенєва" },
      { name: "Федьковича" },
      { name: "Харчука" },
      { name: "Хліборобів" },
      { name: "Чайковського" },
      { name: "Черняховського" },
      { name: "Чехова" },
      { name: "Чорновола" },
      { name: "Шевченка" },
      { name: "Шолом-Алейхема" },
      { name: "Шпитальна" },
      { name: "Яблунева" },
      { name: "Яворницького" },
      { name: "Ярослава Мудрого" },
      { name: "Ярославни" },
    ],
  },
  {
    value: "Зборів",
    name: "Зборів",
    streets: [
      { name: "8 березня" },
      { name: "Базарна" },
      { name: "Богдана Хмельницького" },
      { name: "Богуна" },
      { name: "Вербова" },
      { name: "Верхня" },
      { name: "Верхоставна" },
      { name: "Вишнева" },
      { name: "Галицька" },
      { name: "Гетьманська" },
      { name: "Гоголя" },
      { name: "Гончара" },
      { name: "Грушевського" },
      { name: "Данила Галицького" },
      { name: "Довженка" },
      { name: "Дружби" },
      { name: "Заводська" },
      { name: "Загребельна" },
      { name: "Заозерна" },
      { name: "Зарічна" },
      { name: "Івана Франка" },
      { name: "Івасюка" },
      { name: "Козацька" },
      { name: "Космонавтів" },
      { name: "Котляревського" },
      { name: "Коханівка" },
      { name: "Куклинці" },
      { name: "Купчинського" },
      { name: "Лесі Українки" },
      { name: "Ліпінського" },
      { name: "Львівська" },
      { name: "Медова" },
      { name: "Миру" },
      { name: "Млиновецька" },
      { name: "Набережна" },
      { name: "Надії" },
      { name: "Незалежності" },
      { name: "Паркова" },
      { name: "Перемоги" },
      { name: "Пушкіна" },
      { name: "Садова" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Стуса" },
      { name: "Тернопільська" },
      { name: "Травнева" },
      { name: "Чорновола" },
      { name: "Чумацька" },
      { name: "Шевченка" },
    ],
  },
  {
    value: "Копичинці",
    name: "Копичинці",
    streets: [
      { name: "22 січня" },
      { name: "Байди" },
      { name: "Богуна" },
      { name: "Богдана Хмельницького" },
      { name: "Весела" },
      { name: "Гусятинська" },
      { name: "Довга" },
      { name: "Дорошенка" },
      { name: "Ємелівка" },
      { name: "Заліщицька" },
      { name: "Зелена" },
      { name: "Івана Франка" },
      { name: "Котляревського" },
      { name: "Крута" },
      { name: "Курбаса" },
      { name: "Лепкого" },
      { name: "Лесі Українки" },
      { name: "Мазепи" },
      { name: "Молодіжна" },
      { name: "Паркова" },
      { name: "Польова" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Чайковського" },
      { name: "Чічки Андрієнка" },
      { name: "Чортківська" },
      { name: "Шашкевича" },
      { name: "Шевченка" },
      { name: "Шопена" },
    ],
  },
  {
    value: "Кременець",
    name: "Кременець",
    streets: [
      { name: "107 Кременецької дивізії" },
      { name: "Акімова" },
      { name: "Алея Підгірна" },
      { name: "Базарна" },
      { name: "Березина" },
      { name: "Березова" },
      { name: "Ватутіна" },
      { name: "Вербова" },
      { name: "Вериківського" },
      { name: "Вивозова" },
      { name: "Вишнівецька" },
      { name: "Відродження" },
      { name: "Виговського" },
      { name: "Вокзальна" },
      { name: "Гагаріна" },
      { name: "Гладка" },
      { name: "Глибока" },
      { name: "Глибокі Туники" },
      { name: "Глинянка" },
      { name: "Глінки" },
      { name: "Горбача" },
      { name: "Горького" },
      { name: "Грабівка" },
      { name: "Грушевського" },
      { name: "Джерельна" },
      { name: "Дівича" },
      { name: "Довга" },
      { name: "Драгоманова" },
      { name: "Дружби" },
      { name: "Дубенська" },
      { name: "Замкова" },
      { name: "Затишшя" },
      { name: "Зелена" },
      { name: "Івана Франка" },
      { name: "Климова" },
      { name: "Княжина" },
      { name: "Козацька" },
      { name: "Козубського" },
      { name: "Корнії" },
      { name: "Корольова" },
      { name: "Космонавтів" },
      { name: "Котляревського" },
      { name: "Красногірка" },
      { name: "Крейдяна" },
      { name: "Кривоноса" },
      { name: "Куличівка" },
      { name: "Курчатова" },
      { name: "Лесі Українки" },
      { name: "Липова" },
      { name: "Липовиця" },
      { name: "Ліцейна" },
      { name: "Льотчиків Визволителів" },
      { name: "Ляшуки" },
      { name: "Макова" },
      { name: "Марценівка" },
      { name: "Медова" },
      { name: "Мікрорайон" },
      { name: "Моргви" },
      { name: "Неприцького Грановського" },
      { name: "Нова" },
      { name: "Об'їздова" },
      { name: "Оксани Лятуринської" },
      { name: "Олега Кошового" },
      { name: "Олени Пчілки" },
      { name: "Олени Теліги" },
      { name: "Ольжича" },
      { name: "Осовиця" },
      { name: "Панаса Мирного" },
      { name: "Пасічна" },
      { name: "Переліски" },
      { name: "Першого Травня" },
      { name: "Піддівича" },
      { name: "Підлісна" },
      { name: "Піщана" },
      { name: "Платонової" },
      { name: "Плитниця" },
      { name: "Повздовжня" },
      { name: "Потік Ірва" },
      { name: "Пушкіна" },
      { name: "Садова" },
      { name: "Самчука" },
      { name: "Санаторійна" },
      { name: "Симона Петлюри" },
      { name: "Сичівка" },
      { name: "Сільська" },
      { name: "Спокійна" },
      { name: "Спортова" },
      { name: "Старі Вали" },
      { name: "Старого Детюка" },
      { name: "Стрілецька" },
      { name: "Текстильників" },
      { name: "Тиха" },
      { name: "Толстого" },
      { name: "Туницька" },
      { name: "Харчука" },
      { name: "Чорновола" },
      { name: "Шевченка" },
      { name: "Шкільна" },
      { name: "Шумська" },
      { name: "Ювілейна" },
      { name: "Юліуша Словацького" },
      { name: "Ясна" },
    ],
  },
  {
    value: "Ланівці",
    name: "Ланівці",
    streets: [
      { name: "Берегова" },
      { name: "Білозірська" },
      { name: "Богайчука" },
      { name: "Винниченка" },
      { name: "Вишнева" },
      { name: "Вишнівецька" },
      { name: "Відродження" },
      { name: "Волинська" },
      { name: "Гайова" },
      { name: "Грушевського" },
      { name: "Джерельна" },
      { name: "Драгоманова" },
      { name: "Дружби" },
      { name: "Дубина" },
      { name: "Енергетична" },
      { name: "Заводська" },
      { name: "Залізнична" },
      { name: "Запорізьких козаків" },
      { name: "Зарічна" },
      { name: "Західна" },
      { name: "Зелена" },
      { name: "Івана Франка" },
      { name: "Кармелюка" },
      { name: "Колгоспна" },
      { name: "Корольова" },
      { name: "Коротки" },
      { name: "Котляревського" },
      { name: "Лесі Українки" },
      { name: "Листопадова" },
      { name: "Лугова" },
      { name: "Миру" },
      { name: "Молодіжна" },
      { name: "Морозенка" },
      { name: "Набережна" },
      { name: "Незалежності" },
      { name: "Нова" },
      { name: "Новосілка" },
      { name: "Передова" },
      { name: "Північна" },
      { name: "Підгірна" },
      { name: "Польова" },
      { name: "Поповича" },
      { name: "Привокзальна" },
      { name: "Пушкіна" },
      { name: "Робітнича" },
      { name: "Ровенська" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Східна" },
      { name: "Тернопільська" },
      { name: "Травнева" },
      { name: "Українська" },
      { name: "Хлібна" },
      { name: "Хлібна-бічна" },
      { name: "Шевченка" },
      { name: "Ювілейна" },
    ],
  },
  {
    value: "Монастириська",
    name: "Монастириська",
    streets: [
      { name: "Богдана Хмельницького" },
      { name: "Відродження" },
      { name: "Володимира Великого" },
      { name: "Галицька" },
      { name: "Гнатюка" },
      { name: "Горішня" },
      { name: "Грушевського" },
      { name: "Добровідна" },
      { name: "Долішня" },
      { name: "Зелена" },
      { name: "Злуки" },
      { name: "Івана Франка" },
      { name: "Кисилевської" },
      { name: "Коновальця" },
      { name: "Краснікова" },
      { name: "Лесі Українки" },
      { name: "Мазепи" },
      { name: "Миру" },
      { name: "Нова" },
      { name: "Озерна" },
      { name: "Пекарська" },
      { name: "Перемоги" },
      { name: "Підгородня" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Січових Стрільців" },
      { name: "Степана Бандери" },
      { name: "Стецька" },
      { name: "Стуса" },
      { name: "Тиха" },
      { name: "Тичини" },
      { name: "Хвильового" },
      { name: "Чорновола" },
      { name: "Шевченка" },
      { name: "Шкільна" },
      { name: "Шухевича" },
    ],
  },
  {
    value: "Підгайці",
    name: "Підгайці",
    streets: [
      { name: "Бережанська" },
      { name: "Богдана Хмельницького" },
      { name: "Богуславки" },
      { name: "Боднарська" },
      { name: "Воїнів УПА" },
      { name: "Гунчака" },
      { name: "Дімітрова" },
      { name: "Замкова" },
      { name: "Злуки" },
      { name: "Івана Франка" },
      { name: "Коновальця" },
      { name: "Кравецька" },
      { name: "Лесі Українки" },
      { name: "Лисенка" },
      { name: "Мазепи" },
      { name: "Майдан Незалежності" },
      { name: "Марцелівка" },
      { name: "Мізерного Рена" },
      { name: "Мічуріна" },
      { name: "Молодіжна" },
      { name: "Поповича" },
      { name: "Рудницького" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Спасівська" },
      { name: "Спортивна" },
      { name: "Федьковича" },
      { name: "Чорновола" },
      { name: "Шевченка" },
      { name: "Шептицького" },
      { name: "Шкільна" },
      { name: "Юрдика-горішня" },
      { name: "Яворського" },
    ],
  },
  {
    value: "Почаїв",
    name: "Почаїв",
    streets: [
      { name: "Березина" },
      { name: "Богдана Хмельницького" },
      { name: "Бригадна" },
      { name: "Варшавська" },
      { name: "Верхня" },
      { name: "Винниченка" },
      { name: "Вишнева" },
      { name: "Воз'єднання" },
      { name: "Волинська" },
      { name: "Гагаріна" },
      { name: "Гайова" },
      { name: "Галицька" },
      { name: "Гоголя" },
      { name: "Граби" },
      { name: "Грушевського" },
      { name: "Дачна" },
      { name: "Дикунів" },
      { name: "Діброва" },
      { name: "Дорошенка" },
      { name: "Драли" },
      { name: "Дружби" },
      { name: "Жовтнева" },
      { name: "Жука" },
      { name: "Заводська" },
      { name: "Зайці" },
      { name: "Заньковецької" },
      { name: "Зелена" },
      { name: "Злуки" },
      { name: "Івана Франка" },
      { name: "Квіткова" },
      { name: "Кіндратюка" },
      { name: "Копані" },
      { name: "Космонавтів" },
      { name: "Котляревського" },
      { name: "Кременецька" },
      { name: "Крушельницької" },
      { name: "Лесі Українки" },
      { name: "Липова" },
      { name: "Лісова" },
      { name: "Лосятинська" },
      { name: "Лящука" },
      { name: "Макаренка" },
      { name: "Марка Вовчка" },
      { name: "Матросова" },
      { name: "Медова" },
      { name: "Миру" },
      { name: "Молодіжна" },
      { name: "Монастирська" },
      { name: "Наливайка" },
      { name: "Несова" },
      { name: "Нижня" },
      { name: "Нова" },
      { name: "Озерна" },
      { name: "Орловського" },
      { name: "Остапа Вишні" },
      { name: "Острозького" },
      { name: "Паркова" },
      { name: "Перемоги" },
      { name: "Південна" },
      { name: "Плетянка" },
      { name: "Польова" },
      { name: "Промислова" },
      { name: "Радивилівська" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Сонячна" },
      { name: "Спортивна" },
      { name: "Старотаражська" },
      { name: "Стельмащука" },
      { name: "Стефаника" },
      { name: "Суворова" },
      { name: "Теплична" },
      { name: "Тиха" },
      { name: "Толстого" },
      { name: "Ульріха" },
      { name: "Фабрична" },
      { name: "Церковна" },
      { name: "Чайковського" },
      { name: "Червоноармійська" },
      { name: "Шалівка" },
      { name: "Шатківського" },
      { name: "Шевченка" },
      { name: "Шкільна" },
      { name: "Юридична" },
      { name: "Ярослава Мудрого" },
      { name: "Ясна" },
    ],
  },
  {
    value: "Скалат",
    name: "Скалат",
    streets: [
      { name: "Богдана Хмельницького" },
      { name: "Богуна" },
      { name: "Винниченка" },
      { name: "Вишнівецька" },
      { name: "Вітер" },
      { name: "Грушевського" },
      { name: "Драгоманова" },
      { name: "Замкова" },
      { name: "Івана Франка" },
      { name: "Йосипа Сліпого" },
      { name: "Калнишевського" },
      { name: "Коновальця" },
      { name: "Кривоноса" },
      { name: "Лермонтова" },
      { name: "Лесі Українки" },
      { name: "Леся Курбаса" },
      { name: "Мазепи" },
      { name: "Малишка" },
      { name: "Наливайка" },
      { name: "Незалежності" },
      { name: "Полуботка" },
      { name: "Пулюя" },
      { name: "Сагайдачного" },
      { name: "Садовського" },
      { name: "Симона Петлюри" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Стадіонна" },
      { name: "Степана Бандери" },
      { name: "Стецька" },
      { name: "Стуса" },
      { name: "Тернопільська" },
      { name: "Чорновола" },
      { name: "Чубинського" },
      { name: "Шашкевича" },
      { name: "Шевченка" },
      { name: "Шептицького" },
      { name: "Шухевича" },
    ],
  },
  {
    value: "Теребовля",
    name: "Теребовля",
    streets: [
      { name: "22 січня" },
      { name: "Бічна" },
      { name: "Богдана Хмельницького" },
      { name: "Богуна" },
      { name: "Бойчука" },
      { name: "Будівельників" },
      { name: "Вавілова" },
      { name: "Вербова" },
      { name: "Вузька" },
      { name: "Галицька" },
      { name: "Гжицького" },
      { name: "Гірняка" },
      { name: "Глещавецька" },
      { name: "Грушевського" },
      { name: "Довбуша" },
      { name: "Довженка" },
      { name: "Дружби" },
      { name: "Зазамче" },
      { name: "Залізнична" },
      { name: "Зарчиного" },
      { name: "Застіноцька" },
      { name: "Зелена" },
      { name: "Зої Космодем'янської" },
      { name: "Івана Франка" },
      { name: "Кірова" },
      { name: "Князя Василька" },
      { name: "Кобилянської" },
      { name: "Ковпака" },
      { name: "Коротка" },
      { name: "Косака" },
      { name: "Котляревського" },
      { name: "Коцюбинського" },
      { name: "Кошового" },
      { name: "Крушельницької" },
      { name: "Лесі Українки" },
      { name: "Лисенка" },
      { name: "Ломоносова" },
      { name: "Лошнівська" },
      { name: "Мазепи" },
      { name: "Маковського" },
      { name: "Мандичевського" },
      { name: "Матросова" },
      { name: "Миколаєнка" },
      { name: "Миру" },
      { name: "Навроцького" },
      { name: "Нова" },
      { name: "Паращука" },
      { name: "Паркова" },
      { name: "Пилипа Орлика" },
      { name: "Південна" },
      { name: "Підзамче" },
      { name: "Підлісна" },
      { name: "Плебанівська" },
      { name: "Покрівка" },
      { name: "Польова" },
      { name: "Привокзальна" },
      { name: "Пушкіна" },
      { name: "Рильського" },
      { name: "Руднева" },
      { name: "Садики" },
      { name: "Садова" },
      { name: "Северина Наливайка" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Стрімка" },
      { name: "Стуса" },
      { name: "Тернопільська" },
      { name: "Тиха" },
      { name: "Федьковича" },
      { name: "Хоткевича" },
      { name: "Чайковського" },
      { name: "Чехова" },
      { name: "Шевченка" },
      { name: "Шишкіна" },
      { name: "Ювілейна" },
    ],
  },
  {
    value: "Хоростків",
    name: "Хоростків",
    streets: [
      { name: "Богдана Хмельницького" },
      { name: "Вибранівка" },
      { name: "Висока" },
      { name: "Вузька" },
      { name: "Гагаріна" },
      { name: "Галицька" },
      { name: "Голосна" },
      { name: "Грушевського" },
      { name: "Довга" },
      { name: "Дружби" },
      { name: "Зелена" },
      { name: "Івасюка" },
      { name: "Кармелюка" },
      { name: "Князя Володимира" },
      { name: "Комарова" },
      { name: "Коновальця" },
      { name: "Коцюбинського" },
      { name: "Лисенка" },
      { name: "Миру" },
      { name: "Наливайка" },
      { name: "Незалежності" },
      { name: "Низова" },
      { name: "Ольги Кобилянської" },
      { name: "Палія" },
      { name: "Січових Стрільців" },
      { name: "Сонячна" },
      { name: "Тиха" },
      { name: "Цукрова" },
      { name: "Чорновола" },
      { name: "Шевченка" },
      { name: "Шкільна" },
    ],
  },
  {
    value: "Чортків",
    name: "Чортків",
    streets: [
      { name: "16 липня" },
      { name: "16 липня бічна" },
      { name: "Антонича" },
      { name: "Аптечне" },
      { name: "Байди-Вишневецького" },
      { name: "Барвінського" },
      { name: "Бердо" },
      { name: "Березовського" },
      { name: "Білецька" },
      { name: "Богуна" },
      { name: "Броварова" },
      { name: "Бучацька" },
      { name: "Бучацька бічна" },
      { name: "Вербицького" },
      { name: "Вербова" },
      { name: "Весняна" },
      { name: "Вербова" },
      { name: "Весняна" },
      { name: "Вигнанська" },
      { name: "Виговського" },
      { name: "Вишнева" },
      { name: "Вільшана" },
      { name: "Вітовського" },
      { name: "Вітовського бічна" },
      { name: "Водна" },
      { name: "Вокзальна" },
      { name: "Гагаріна" },
      { name: "Галицька" },
      { name: "Гірська" },
      { name: "Глибока" },
      { name: "Глуха" },
      { name: "Гніздовського" },
      { name: "Гоголя" },
      { name: "Гончара" },
      { name: "Горбачевського" },
      { name: "Гранична" },
      { name: "Гранична бічна" },
      { name: "Гранична Нова" },
      { name: "Грушевського" },
      { name: "Джерельна" },
      { name: "Довбуша" },
      { name: "Дорошенка" },
      { name: "Драгоманова" },
      { name: "Заводська" },
      { name: "Залізнична" },
      { name: "Замкова" },
      { name: "Зелена" },
      { name: "Золотарка" },
      { name: "Івасюка" },
      { name: "Каденюка" },
      { name: "Карабіневича" },
      { name: "Кармелюка" },
      { name: "Київська" },
      { name: "Князя Володимира Великого" },
      { name: "Кобилянської" },
      { name: "Коновальця" },
      { name: "Копичинецька" },
      { name: "Копичинецька бічна" },
      { name: "Корольова" },
      { name: "Коротка" },
      { name: "Котляревського" },
      { name: "Коцюбинського" },
      { name: "Кривоноса" },
      { name: "Криницького" },
      { name: "Крушельницької" },
      { name: "Левицького" },
      { name: "Лемківська" },
      { name: "Леонтовича" },
      { name: "Лепких" },
      { name: "Лепкого" },
      { name: "Лисенка" },
      { name: "Лісова" },
      { name: "Мазепи" },
      { name: "Маковея" },
      { name: "Маньовського" },
      { name: "Мельника" },
      { name: "Мельничука" },
      { name: "Мистецька" },
      { name: "Млинарська" },
      { name: "Молодіжна" },
      { name: "Монастирська" },
      { name: "Монастирська бічна" },
      { name: "Надбережна" },
      { name: "Надрічна" },
      { name: "Наливайка" },
      { name: "Незалежності" },
      { name: "Нова" },
      { name: "Носса" },
      { name: "Об'їзна" },
      { name: "Об'їзна бічна" },
      { name: "Огієнка" },
      { name: "Ольжича" },
      { name: "Орлика" },
      { name: "Паркова" },
      { name: "Петрушевича" },
      { name: "Пігута" },
      { name: "Підгірна" },
      { name: "Підгірна Нова" },
      { name: "Підлісна" },
      { name: "Пітушевського" },
      { name: "Подільська" },
      { name: "Полуботка" },
      { name: "Польова" },
      { name: "Проїзна" },
      { name: "Пулюя" },
      { name: "Рильського" },
      { name: "Ринок" },
      { name: "Росляка" },
      { name: "Рубчакової" },
      { name: "Рудькова" },
      { name: "Савки" },
      { name: "Сагайдачного" },
      { name: "Садова" },
      { name: "Середня" },
      { name: "Сингаївського" },
      { name: "Синенького" },
      { name: "Сірка" },
      { name: "Січинського" },
      { name: "Січових Стрільців" },
      { name: "Сковороди" },
      { name: "Сліпого" },
      { name: "Сонячна" },
      { name: "Сонячна Верхня" },
      { name: "Стефаника" },
      { name: "Стрімка" },
      { name: "Стуса" },
      { name: "Тарнавського" },
      { name: "Теліги" },
      { name: "Теребовлянська" },
      { name: "Тудора" },
      { name: "Удовиченка" },
      { name: "Українки" },
      { name: "Франка" },
      { name: "Хичія" },
      { name: "Хмельницького" },
      { name: "Церковна" },
      { name: "Чайковського" },
      { name: "Червоний Берег" },
      { name: "Чортківська" },
      { name: "Чубинського" },
      { name: "Шашкевича" },
      { name: "Шеремети" },
      { name: "Шкільна" },
      { name: "Шопена" },
      { name: "Шопена бічна" },
      { name: "Шухевича" },
      { name: "Юрчинських" },
      { name: "Ягільницька" },
      { name: "Яремчука" },
      { name: "Ясна" },
    ],
  },
  {
    value: "Шумськ",
    name: "Шумськ",
    streets: [
      { name: "840-річчя Шумська" },
      { name: "Будівельників" },
      { name: "Богдана Хмельницького" },
      { name: "Вишневий" },
      { name: "Вишнева" },
      { name: "Вілійська" },
      { name: "Волинська" },
      { name: "Гагаріна" },
      { name: "Добровінського" },
      { name: "Дружби" },
      { name: "Данилевича" },
      { name: "Енергетична" },
      { name: "Жовтнева" },
      { name: "Заводська" },
      { name: "Зелена" },
      { name: "Івана Франка" },
      { name: "Котляревського" },
      { name: "Князя Острозького" },
      { name: "Лесі Українки" },
      { name: "Миру" },
      { name: "Незалежності" },
      { name: "Народицька" },
      { name: "Набережна" },
      { name: "Озерна" },
      { name: "Промислова" },
      { name: "Підгірна" },
      { name: "Садова" },
      { name: "Сонячна" },
      { name: "Стадіонна" },
      { name: "Свободи" },
      { name: "Українська" },
      { name: "Уласа Самчука" },
      { name: "Чирянська" },
      { name: "Шевченка" },
      { name: "Ювілейна" },
    ],
  },
];
export const districtList = [
  { value: "", name: "оберіть район" },
  { value: "Тернопільський район", name: "Тернопільський" },
  { value: "Кременецький район", name: "Кременецький" },
  { value: "Чортківський район", name: "Чортківський" },
];
export const usefulLinks = [
  {
    title: "Перевірити лікарняний",
    link: "https://portal.pfu.gov.ua/sidebar/Templates/IncapListsSearch",
    iconName: "trueAnswer",
    color: "green"
  },
  {
    title: "Код-помічник",
    link: "https://olehkuts.github.io/code_helper/",
    iconName: "helper",
    color: "black"
  },
  {
    title: "Довідка до школи",
    link: "https://OlehKuts.github.io./medical_certification",
    iconName: "document",
    color: "sienna"
  },
  {
    title: "Направлення на аналізи",
    link: "https://OlehKuts.github.io./analyses_directions",
    iconName: "examination",
    color: "coral"
  },
  {
    title: "Операційний список",
    link: "https://OlehKuts.github.io./operational_list",
    iconName: "list",
    color: "black"
  },
  {
    title: "Календар для операцій",
    link: "https://OlehKuts.github.io./upgraded_calendar",
    iconName: "calendar",
    color: "royalblue"
  }
];

export const diaryListTemplate = [
  {
    date: "12.1.2024",
    workDay: true,
    id: Math.random(),
    localStatus: "Обличчя симетричне",
    generalStatus: "Загальний стан дитини.",
  },
  {
    date: "13.1.2024",
    workDay: false,
    id: Math.random(),
    localStatus: "Обличчя симетричне",
    generalStatus: "Загальний стан дитини.",
  },
  {
    date: "14.1.2024",
    workDay: false,
    id: Math.random(),
    localStatus: "Обличчя симетричне",
    generalStatus: "Загальний стан дитини.",
  },
  {
    date: "15.1.2024",
    workDay: true,
    id: Math.random(),
    localStatus: "Виписується",
    generalStatus: "Загальний стан дитини задовільний.",
  },
];
export const dayTemplate = [
  {
    date: "1.1.2024",
    generalStatus: "Загальний стан",
    localStatus: "Стан щелепно-лицевої ділянки...",
    id: Math.random(),
    workDay: false,
  },
];
